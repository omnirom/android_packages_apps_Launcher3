...
public class SearchLauncherCallbacks implements LauncherCallbacks,
    ...
    private final SearchLauncher mSearchLauncher;
    ...
    public SearchLauncherCallbacks(SearchLauncher launcher) {
        mSearchLauncher = launcher;
    }
    ...
    public void onCreate(Bundle savedInstanceState) {
        SharedPreferences prefs = Utilities.getPrefs(mSearchLauncher);
        mOverlayCallbacks = new OverlayCallbackImpl(mSearchLauncher);
        mLauncherClient = new LauncherClient(mSearchLauncher, mOverlayCallbacks, getClientOptions(prefs));
        mOverlayCallbacks.setClient(mLauncherClient);
        prefs.registerOnSharedPreferenceChangeListener(this);

        mSearchLauncher.addOnDeviceProfileChangeListener(this);
    }
    ...
    public void onHomeIntent(boolean internalStateHandled) {
        mLauncherClient.hideOverlay(mSearchLauncher.isStarted() && !mSearchLauncher.isForceInvisible());
    }

    @Override
    public void onResume() {
        Handler handler = mSearchLauncher.getDragLayer().getHandler();
        if (mDeferCallbacks) {
            if (handler == null) {
    ...
    private void checkIfStillDeferred() {
        ...
        if (!mSearchLauncher.hasBeenResumed() && mSearchLauncher.isStarted()) {
            return;
        }
        mDeferCallbacks = false;

        // Move the client to the correct state. Calling the same method twice is no-op.
        if (mSearchLauncher.isStarted()) {
            mLauncherClient.onStart();
        }
        if (mSearchLauncher.hasBeenResumed()) {
            mLauncherClient.onResume();
        } else {
            mLauncherClient.onPause();
        }
    ...
    public void onDestroy() {
        ...
        Utilities.getPrefs(mSearchLauncher).unregisterOnSharedPreferenceChangeListener(this);
    }
